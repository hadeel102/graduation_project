    ...
    public class ProductController : Controller
    {
        ...
        public ActionResult Search(FormCollection oform)
        {
            ...
            using (var db = new ECSalesEntities2())
            {
                ...
                ViewBag.list = db.products.Include("TblCategory").Where(x => x.IsService == false && x.UserID == UserID
                 && (SearchEname == "" || x.Ename.Contains(SearchEname))
                   && (SearchPrice == "" || x.Price == dSearchPrice)
                 ).ToList();
                var dataCategory = db.TblCategories.Where(
                     x => x.IsService == false && x.ParentID != null
                     ).ToList();
        ...
        public ActionResult Index()
        {
            ...
            {
                var UserID = (Session["UserInfo"] as TblUser).ID;
                ViewBag.list = db.products.Include("TblCategory").Where(x => x.IsService == false && x.UserID == UserID).ToList();
        ...
        public ActionResult SaveProduct(vmproduct oproduct)
        {
            using (var db = new ECSalesEntities2())
            {
                if (oproduct.ID <= 0)
                {
                    ...
                    UploadPath = Server.MapPath("~/Uploads/Products/") + FileName + FileExtension;
        ...
        {
            //Build the File Path.
            string path = Server.MapPath("~/Uploads/Products/") + AttchedFile;
        ...
        [HttpPost]
        public ActionResult GetByIdHistory(int id)
        {

            product oProduct = new product();
            product ooldProduct = new product();
            using (var db = new ECSalesEntities2())
            {
                oProduct = db.products.Where(x => x.ID == id).FirstOrDefault();
                ooldProduct = JsonConvert.DeserializeObject<product>(oProduct.OldData);
            }
            ...
            return Json(new
            {
                data = new product
                {
                    Aname = ooldProduct.Aname,
                    ID = ooldProduct.ID,
                    CategoryID = ooldProduct.CategoryID,
                    Description = ooldProduct.Description,
                    Ename = ooldProduct.Ename,
                    Image1 = ooldProduct.Image1,
                    IsService = ooldProduct.IsService,
                    Price = ooldProduct.Price,
                    TotalQty = ooldProduct.TotalQty
                }
            }, JsonRequestBehavior.AllowGet);
        }
